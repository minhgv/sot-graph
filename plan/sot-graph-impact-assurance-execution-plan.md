...
- [x] Truyền active `Database` vào provider qua orchestrator (W1); persist run/binding/evidence trên production CLI+MCP path.
  — Receipt: `federation_plan(..., db=)` → `CodebaseMemoryProvider(db=db)`; `federated_extras(..., db=)`; CLI `usages`/`explore`/`diff-impact` truyền `db=db` (3 sites). Evidence persistence kích hoạt tại điểm trung chuyển `_invoke` (`_persist_evidence`): mọi query CBM (trace_path/search_graph/...) ghi `provider_evidence` cạnh run, sidecar-isolated (exception → warning, không abort query). MCP write path: `McpService.providers_sync` + tool `sot_providers_sync` (write-lock guarded, mirrors `sot providers sync`); read tools giữ mode=ro.
- [x] Commit run + evidence atomically sau parse/verify; lưu version/capability/command digest/duration/status/snapshot.
  — Receipt: `_persist_run` (version/capability/command_digest/duration/status/snapshot_hash) + `_persist_evidence` ngay sau outcome shaping trong cùng query; index run (`index_repository`) giờ bind snapshot: sau run đọc lại `index_status` → UPDATE snapshot_hash + record_provider_binding. `tests/test_p6_ledger.py::test_usages_writes_run_and_evidence` + `test_providers_sync_records_run_and_evidence` (stub binary wire-faithful MCP envelope).
- [x] Union theo canonical identity + relation + target + snapshot; giữ provenance support/contradict từng provider; không trộn historic stale run vào active; conflict adjudication ưu tiên current source/compiler, chưa đủ → giữ `CONFLICT`.
  — Receipt: `assurance/ledger.py::union_evidence` — group key (path+language, relation, src, dst, snapshot) dùng `_language_of` từ P4 identity; chỉ runs `status='ok'` (failed runs loại — test); mỗi entry giữ `providers` set (codebase-memory + scip cùng identity → SUPPORTED, 2 providers). Adjudication: spans mâu thuẫn → verify từng span trên current source; đúng 1 VERIFIED → `source_verified` + resolved_span; 0 hoặc >1 → giữ `CONFLICT` (tests `test_conflict_adjudication_no_false_verified` — false-verified-edge=0 vì resolved_span là span thật đã verify; `test_unresolvable_conflict_stays_conflict`).
- [x] Purge một run không mất evidence run khác; ledger failure không corrupt builtin graph (sidecar isolation).
  — Receipt: `purge_provider_run` cascade chỉ run_id đích (test `test_purge_one_run_keeps_others`); mọi ledger write bọc try/except warning — builtin graph không bao giờ bị hỏng bởi ledger (P2 `test_adapter_survives_exploding_ledger` + `_persist_evidence` sidecar isolation).
- [x] Exit gate: 1 query CLI + 1 query MCP thật tạo ledger rows có snapshot; tái tạo receipt từ ledger không cần log console; không winner-takes-all âm thầm.
  — Receipt: CLI `usages --provider auto` qua stub → runs trace_path ok + snapshot_hash + evidence rows; MCP `providers_sync` → run + snapshot + binding; `receipt_from_ledger(db, root, run_ids)` replay toàn bộ (runs/union/conflicts/adjudication) chỉ từ SQLite; union không thay đổi verdict builtin (read-side) và CONFLICT không bao giờ tự chọn winner. 2 pending gates từ P4 đóng: false-verified-edge=0 (resolved_span luôn là span verify được trên source hiện tại) + union không giảm verified-precision (union là lớp read-side thêm provenance, không hạ verdict — khóa bằng test).

## P7 — Impact engine & receipts (R7)

- [x] Scope receipt trước thay đổi: request identity + resolved target, snapshot ID/digest, source anchors, direct callers/callees, imports/implementations/inheritance, transitive bounded impact, affected files/modules, candidate tests, providers/runs/versions, coverage/exclusions, conflicts/truncation/gaps, assurance status, OMP confirmations còn lại.
  — Receipt: `assurance/receipts.py::scope_receipt(db, root, target, depth, kind_of_change, touches_auth, dynamic_heavy)` — đầy đủ field families: request/resolved_target (P4 identity row), snapshot (`assured_query_context`: snapshot_id/commit_sha/dirty/descriptor_digest), source_anchors, direct_callers/direct_callees (graph_edges 2 chiều), relations imports/inheritance (extends/implements), transitive_impact bounded (explore_node depth + truncated flag), affected_files, candidate_tests (test-path heuristic), providers (P6 ledger cross-check: runs/union_entries/open_conflicts), coverage (P5 note/basis/gaps), assurance (risk + rename_gate + omp_confirmations + status). CLI: `sot scope-receipt <target> [--change-kind] [--auth] [--dynamic] [--json]`. Test `test_field_families_present`.
- [x] Diff-impact receipt sau thay đổi: diff identity (base/head/index/worktree), changed files/symbols, upstream/downstream, invalidated pre-change evidence, tests cần chạy, reconcile result + post-change snapshot, remaining gaps, closure decision.
  — Receipt: `diff_impact_receipt(...)` bọc `analyze_diff_impact` (P3 engine: changed_files/direct_nodes/caller_impacts upstream/api_impacts/test_impacts) + diff_identity (target/staged/working_tree), invalidated_evidence (provider_evidence rows theo changed paths), tests_to_run, reconcile.required + post_change_snapshot (role=post_change), pre_receipt_digest (cross-ref), remaining_gaps, closure_decision (open/closed). Tests `TestDiffReceipt`.
- [x] Assurance rules theo rủi ro (bảng roadmap §R7.3): local body → `verify`; public API/rename/delete → `audit`; auth/tenant → `audit` + security reviewer; dynamic-heavy → không absence assurance; thêm type-surface (class/interface) → audit.
  — Receipt: `classify_change_risk` trả {level, security_reviewer, absence_assurance, rule}; `omp_confirmations_for` sinh checklist operator còn nợ (gate blocked / security sign-off / no-absence / targeted tests). Test `test_r7_3_table` khóa toàn bộ nhánh.
- [x] Receipt JSON có schema version + deterministic digest; pre-change receipt không dùng làm proof post-change.
  — Receipt: `RECEIPT_SCHEMA_VERSION=1`; `receipt_digest` = sha256 canonical JSON (sort_keys, no spaces) — deterministic (test 2 lần bằng nhau, đổi content → khác); `proof_scope: "pre_change_only"` trên scope receipt, `"post_change"` trên diff receipt; diff receipt luôn bind post_change snapshot riêng, pre digest chỉ là cross-reference. Test `test_digest_deterministic_and_sensitive` + `test_post_change_receipt_shape`.
- [x] Exit gate: public rename bị block khi caller coverage chưa đủ; `0 callers` chỉ xuất hiện trong bounded assured scope; đối chiếu được với ledger P6.
  — Receipt: `check_rename_gate` — 0 callers + coverage dưới sàn (0.9 tổng + scoped) → BLOCKED với lý do "absence only holds within a bounded, measured scope"; target không resolve → BLOCKED; CLI exit code 2 khi BLOCKED (test). Scope receipt `providers` block đọc thẳng P6 ledger (runs/union/conflicts) — đối chiếu được. Tests `TestRenameGate` + `test_rename_blocked_exit_code`.
